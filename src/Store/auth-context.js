...
export function HerbalContextProvider( { children }) {
    ...
    const [currentTime, setCurrentTime] = useState(new Intl.DateTimeFormat('en-us', {hour: 'numeric', minute: 'numeric'}).format(currentDate));
    ...
    function openCloseStatus(status) {
        ...
        if((time >= 1000 && timeArray[1] === 'AM') || (time <= 1159 && timeArray[1] === 'AM')) {
            setOpen(true);
        }
    ...
    return <HerbalContext.Provider value={{ 
        getWeekday,
        openCloseStatus, 
        setOpen, 
        weekday, 
        currentTime, 
        open }}>{children}</HerbalContext.Provider>
}
