...
function Hero({ navState, heroElement }) {
  ...
  return (
    <>
      <ContainerXL>
          <section className={classes.hero} style={{}} ref={heroElement} style={{backgroundImage: `url(${heroBG})`}} aria-label="Marijuana leaves">

            <div className={classes['gradient']}></div>

            {/* Overlay when mobile menu is active */}
            <div className={`${!navState ? classes['menuActive'] : classes['d-none']}`}></div>
              ...
              <div className={classes['hero-wrapper']}>
                  ...
                  <div className={classes['hero-btn--wrapper']}>
                      <Button className={`hero-btn--green safari-gap--fix1_5 ${classes['hero-btn']}`}>
                          <Link to="/menu">Order Now <span>&rarr;</span></Link>
                      </Button>

                      <Button className={`hero-btn--blue ${classes['hero-btn']}`}>
                          <Link to="/contact-us">Contact us <span>&rarr;</span></Link>
                      </Button>
                  </div>
              </div>
           </section>
